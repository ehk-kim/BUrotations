...
basePath = './';    
rawTranscriptomeFasta = [basePath 'Mus_musculus.GRCm39.cdna.all.fa'];
fpkmPath = [basePath 'abundances_m.mat'];
ncRNAPath = [basePath 'Mus_musculus.GRCm39.ncrna.fa'];
readoutPath = [basePath 'readouts.fasta'];
codebookPath = [basePath 'codebook_cm1_all49.csv'];
...
if ~exist(isoSpecificityTablePath)
    ...
    for i=1:length(names) % Loop over all gene names -- RNAs that share the same gene name are considered isoforms
        ...
        display('Local transcriptome generated. Starting OTTable')
...
%if ~exist(specificityTablePath)
    ...
    % Add isoform specificity ta bles to create total transcriptome
    % specificity table
...
if ~exist(trDesignerPath)
    TRDesignerWithoutByteStream = TRDesigner('transcriptome', slicedTranscriptome, ...
        'OTTables', [OTrRNA15], ...
        'OTTableNames', {'rRNA'}, ...
        'specificityTable', specificityTable, ...
        'isoSpecificityTables', isoSpecificityTables, ...
        'parallel', p);
    TRDesignerWithoutByteStream.Save(trDesignerPath);
else
    TRDesignerWithoutByteStream = TRDesigner.Load(trDesignerPath);
end
...
%%-------------------------------------------------------------------------
% --- DIFFERENCE ---
% these are code chunks that are specific to each probe set. this part should be easily accessible to edit
if ~exist(trRegionsPath)
	% Design target regions
	targetRegions = TRDesignerWithoutByteStream.DesignTargetRegions(...
		'regionLength', 30, ...
		'GC', [43 63]/100, ...
		'Tm', [66 76], ...
...
load('/projectnb/perturb-fish/ekim/MERFISH_analysis/Tmaxstr_49.mat');
...
if ~exist(oligosPath)
    ...
    for i=1:length(finalIds)
        ...
        if ~isempty(tRegion) % Check to see if there are no target regions--only used for blanks
            ...
            for it=1:tRegion.numRegions
                % Create random orientation and selection of readouts
                localReadouts = possibleReadouts(randperm(length(possibleReadouts), 2));
                %if rand(1) > 0.5
                if rand(1) > 0
                    % Create header 
                    headers{it} = [libraryName ' ' ...
                        localReadouts(1).Header ' ' ...
                        tRegion.geneName '__' ...
                        tRegion.id '__' ...
                        num2str(tRegion.startPos(it)) '__' ...
                        num2str(length(tRegion.sequence{it})) '__' ...
                        num2str(tRegion.GC(it)) '__' ...
                        num2str(tRegion.Tm(it)) '__' ...
                        num2str(tRegion.specificity(it)) ' ' ...
                        localReadouts(2).Header ' ']; ...

                    % Create sequence
                    seqs{it} = ['A ' seqrcomplement(localReadouts(1).Sequence) ' '...
                            seqrcomplement(tRegion.sequence{it}) ' A ' ...
                    ...
                    seqs{it} = ['A ' seqrcomplement(localReadouts(1).Sequence) ' ' ...
                        ...
                        seqrcomplement(tRegion.sequence{it}) ' A ' ...
                            ...
                            seqrcomplement(localReadouts(4).Sequence)];
                end
            end
